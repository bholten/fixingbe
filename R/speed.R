...
# Speed
model <- glm(speed ~ courage, data = combined_data, family = gaussian())
summary(model)
...
combined_data$predicted_speed <- predict(model, newdata = combined_data, type = "response")
combined_data$predicted_speed_error <- combined_data$speed - combined_data$predicted_speed

ggplot(combined_data, aes(x = speed, y = predicted_speed)) +
  geom_point() +
  geom_abline(intercept = 0, slope = 1, linetype = "dashed", color = "red") +
  theme_minimal() +
  xlab("Actual Speed") +
  ylab("Predicted Speed") +
  ggtitle("Comparison of Actual vs Predicted Speed")
